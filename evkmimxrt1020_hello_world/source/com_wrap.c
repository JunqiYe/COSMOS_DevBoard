...
static bool enterCommandMode()
 {
	...
	size_t n = 0;
	int returnVal =  LPUART_RTOS_Send(&LPUART2_PERIPHERAL, (uint8_t *)tx_buffer, 3); //LPUART_RTOS_Send(&LPUART2_PERIPHERAL, (uint8_t *)tx_buffer, 3);
    PRINTF("message sent\n");
...
static bool sendConfigCommand(uint8_t data[], uint8_t expectedResponse[], int sizeofTx, int sizeExpectedResponse) {
    ...
    while (try < DEFAULT_RETRIES) {
    	...
    	int sendReturnVal = LPUART_RTOS_Send(&LPUART2_PERIPHERAL, (uint8_t *)tx_buffer, sizeofTx); //LPUART_RTOS_Send(&LPUART2_PERIPHERAL, tx_buffer, sizeofTx); // Rithu: changing to sizeOfTx
        ...
        int recReturnVal = LPUART_RTOS_Receive(&LPUART2_PERIPHERAL, rx_buffer, sizeof(rx_buffer), size_t);
...
void com_getCommands() //highest priority
{
	...
	if (kStatus_Success != LPUART_RTOS_Send(&LPUART2_PERIPHERAL, (uint8_t *)to_send, strlen(to_send)))//LPUART_RTOS_Send(&LPUART2_PERIPHERAL, (uint8_t *)to_send, strlen(to_send)))
	{
		PRINTF("could not send!!!\r\n\r\n");
		return;
	}

	status_t error = LPUART_RTOS_Receive(&LPUART2_PERIPHERAL, rcv_buffer, sizeof(rcv_buffer), &n);
	...
	if (n > 0)
	{
		...
		if (kStatus_Success != LPUART_RTOS_Send(&LPUART2_PERIPHERAL, (uint8_t *)rcv_buffer, n))//LPUART_RTOS_Send(&LPUART2_PERIPHERAL, (uint8_t *)rcv_buffer, n))
		{
			vTaskSuspend(NULL);
		}
	}
...
//	    if (kStatus_Success != LPUART_RTOS_Send(&LPUART2_PERIPHERAL, (uint8_t *)to_send, strlen(to_send))){
//	    	vTaskSuspend( NULL );
//	    }
	    PRINTF("message sent\n");
	    ...
	    int n = 0;
	    status_t error = LPUART_RTOS_Receive(&LPUART2_PERIPHERAL, receive_buffer, 8, &n);
	    do
	    {
	    	error = LPUART_RTOS_Receive(&LPUART2_PERIPHERAL, receive_buffer, 8, &n);
	        ...
	        if (error == kStatus_LPUART_RxHardwareOverrun)
	        {
	            ...
	            if (kStatus_Success !=
					LPUART_RTOS_Send(&LPUART2_PERIPHERAL, (uint8_t *)send_hardware_overrun, strlen(send_hardware_overrun)))
	            {
	                vTaskSuspend(NULL);
	            }
	        }
	        if (error == kStatus_LPUART_RxRingBufferOverrun)
	        {
	            ...
	            if (kStatus_Success != LPUART_RTOS_Send(&LPUART2_PERIPHERAL, (uint8_t *)send_ring_overrun, strlen(send_ring_overrun)))
	            {
	                vTaskSuspend(NULL);
	            }
	        }
	        ...
	        {
	            /* send back the received data */
	            if (kStatus_Success != LPUART_RTOS_Send(&LPUART2_PERIPHERAL, (uint8_t *)receive_buffer, 8))
	            {
	                vTaskSuspend(NULL);
	            }
	        }
